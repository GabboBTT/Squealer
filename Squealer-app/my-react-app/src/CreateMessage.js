...
import {getUsers, updateUsers, updateUser, updateChannels, getListChannels, getUserById, getListSqueals, getRandomTweet, getActualUser, addSqueal, addChannel, updateChannel} from './serverRequests.js';
...
function CreateMessage(props) {
    ...
    useEffect(() => {
     async function fetchUserData() {
      try {
        ...
        if (userData.char_d < userData.char_w) {
          if (userData.char_d < userData.char_m) {
            setMaxChar(userData.char_d);
          } else if (userData.char_m < userData.char_w) {
            setMaxChar(userData.char_m);
          } else {
            setMaxChar(userData.char_w);
          }
        } else if (userData.char_w < userData.char_m) {
          setMaxChar(userData.char_w);
        } else {
          setMaxChar(userData.char_m);
        }
  ...
  const controlChannel = () => {
    ...
    const foundChannel = allkeywords.find(channel => channel.name === textWithoutHashtag);
    //const foundChannel = getListChannels().find(channel => (channel.name === textWithoutHashtag && channel.type === '#'));
    if (foundChannel) {
  ...
  const handleSendChannelSqueal = async () => {
    ...
    if (channelSelected) {
      ...
      try {
        ...
        const textChars = squealChatTextareaValue.length; 
        let imageChars = 0;
        let videoChars = 0;
        let linkChars = 0;
        let positionChars = 0;
  ...
  const handleSendChannelDefaultSqueal = async (defaultCamp) => {
	...
	goToProfile();
        ...
        localStorage.setItem('Interval active', 'false');
  ...
  const styleButtonBottom = windowSize < 426 ? { marginBottom: "30% !important" } : { };
    ...
    return (
      <>
     	{(disabledSendButton && (!(messageType === 'Channel' && squealOrChannelOption === 'Create')) && ((!(localStorage.getItem('Interval active') === 'true')) && (!(localStorage.getItem('Interval active') === 'false')))) ? (
	<>
                ...
                {(messageType === 'Squeal' && squealOrChannelOption === 'Privato') && (
                  <>
                    ...
                    <Row className="mt-2" style = {{marginLeft: '6%'}} id = "allegatiConainer"> 
                      ...
                      {/* Colonna per il pulsante Invia, allineata a destra */}
                      <Col className="d-flex justify-content-end" md={2} id = "sendMessage">
                        <Button onClick={handleSendPrivateSqueal} id = "buttonSend" disabled = {disabledSendButton} style={{ marginBottom: windowSize < 426 ? "30%" : undefined }}>Send</Button>
                      </Col>
                    ...
                    <>
                      ...
                      {(showDefaultMessage === false) &&
                        <Row className="mt-2" style = {{marginLeft: '6%'}} id = "textAreaContentContainer"> 
                          ...
                          {/* Colonna per il pulsante Invia, allineata a destra */}
                          <Col className="d-flex justify-content-end" md={2} id = "sendMessage">
                            <Button onClick={handleSendChannelSqueal} id = "buttonSend" disabled = {disabledSendButton} style={{ marginBottom: windowSize < 426 ? "30%" : undefined }}>Send</Button>
                          </Col>
                        </Row>
                      }
                      ...
                      {isDefaultMessageValid &&
                        <Col className="col-1">
                          <Button onClick={processDefaultMessageType} id = "buttonSend" disabled = {disabledSendButton} style={{ marginBottom: windowSize < 426 ? "30%" : undefined }}>Send</Button>
                        </Col> 
                      }
                    </>
                )}
