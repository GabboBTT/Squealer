...
import {getUsers, getListChannels, getUserById, deleteUsers, getListSqueals, getActualUser, updateUsers, updateUser, updateChannel, updateSqueal, addUser, addSqueal, addChannel, deleteChannel, deleteSqueal} from './serverRequests.js';
...
function Profile() {
                        ...
                        if(allSquealsprint[indexsquealtodelete].channel===allchannels[i].name){
                            for(let j=0;j<allchannels[i].list_posts.length;j++){
                                if((allSquealsprint[indexsquealtodelete].sender==allchannels[i].list_posts[j].sender)&&(allSquealsprint[indexsquealtodelete].date==allchannels[i].list_posts[j].date)&&(allSquealsprint[indexsquealtodelete].hour==allchannels[i].list_posts[j].hour)&&(allSquealsprint[indexsquealtodelete].seconds==allchannels[i].list_posts[j].seconds)){
                                    listnewchannels[i].list_posts.splice(j,1);
                                }
                            }
                        }
                    }
                        ...
                        if(allSquealsprint[indexsquealtodelete].channel===allkeywords[i].name){
                            for(let j=0;j<allkeywords[i].list_posts.length;j++){
                                if((allSquealsprint[indexsquealtodelete].sender==allkeywords[i].list_posts[j].sender)&&(allSquealsprint[indexsquealtodelete].date==allkeywords[i].list_posts[j].date)&&(allSquealsprint[indexsquealtodelete].hour==allkeywords[i].list_posts[j].hour)&&(allSquealsprint[indexsquealtodelete].seconds==allkeywords[i].list_posts[j].seconds)){
                                    listnewkeywords[i].list_posts.splice(j,1);
                                }
                            }
                        }
                    }
    ...
    const closeeditchannel = async (save) => {
	    ...
	    let ind = ListChannelsToUpdate.findIndex(item => item._id === allChannelsprint[indexchanneledit]._id);
            await updateAllChannels(ListChannelsToUpdate, ind);
        }
    ...
    const deletechannel = async (confirm) => {
	    ...
	    let SquealsUpdated = [];
        	for(let i=0; i<SquealsToUpdate.length; i++){
		     for(let j=0; j<allChannelsprint[indexchanneledit].list_posts.length; j++){
            		if((SquealsToUpdate[i].sender!=allSquealsprint[indexsquealtodelete].list_posts[j].sender) || (SquealsToUpdate[i].date!=allSquealsprint[indexsquealtodelete].list_posts[j].date) || (SquealsToUpdate[i].hour!=allSquealsprint[indexsquealtodelete].list_posts[j].hour) || (SquealsToUpdate[i].seconds!=allSquealsprint[indexsquealtodelete].list_posts[j].seconds)){
        ...
        if(confirm){
                switch(selection){
                    ...
                    case "Repeat":
                        ...
                        if(j==false){
                            if((numSeconds!="")&&(parseInt(numSeconds)>5)){
                                setnewchannelmessages(prevmess => ([...prevmess, {
                                    body: {
                                        text: "to /begin",
                                        position: [],
                                ...
                                alert('Insert the number of seconds for repeat (>5s)');
                            }
                        } else {
  ...
  const handleVideoChange = (e) => {
    ...
    return (
        ...
        <Container style={{ width: windowSize>=1024 ? '80%': windowSize>=600 ? '90%' : '100%', left:windowSize>=1024 ? '20%': windowSize>=600 ? '10%' : '0', height: windowSize>=600 ? '100vh' : '90%', position:'absolute', alignItems: 'center', overflow:'hidden'}} className="d-flex flex-column">
            ...
            <Container className="text-center text mt-5" style={{overflowY:'scroll', height:'400px'}}>
                ...
                <Col key={index} className='mt-3'>
                    <Card style={{backgroundColor:'white',  borderColor:'white', minWidth:'280px', minHeight:'200px', marginBottom:index===(allSquealsprint.length-1) ? '100px' : '5%'}}>
                        <Card.Header className='d-flex' style={{justifyContent:'space-between', flexWrap:'wrap'}}>
                            ...
                            { actualuser.photoprofile!='' ? (<div className='me-3' style={{width:'30px', minWidth:'30px', height:'30px', borderRadius:'50%', border:'2px solid black', display:'flex', alignItems:'center', overflow:'hidden'}}>
                        ...
                        <Card.Text style={{textAlign:'left'}}>
                                ...
                                {squeal.body.video!='' && (
                                <div style={{ position: 'relative', width: '200px', maxHeight: '200px', overflow: 'hidden' }}>
                                    <video src={squeal.body.video} alt="Squeal video" width="200px" height="100px" controls/>
                                </div>
                                )}
                ...
                {allChannelsprint.map((channel,index) => (
                    <Row key={index} onClick={() => editchannel(index)} className='d-flex rowchannel_profile' style={{justifyContent:'center',alignItems:'center',cursor:'pointer', borderRadius:'12px', padding:'5px'}}>
                        <div style={{width:'80px',height:'80px', display:'flex', padding:'0', alignItems:'center', borderRadius:'50%', border:'2px solid black', overflow:'hidden'}}>
                        ...
                        : (<Image src={'/squealer-app'+channel_profile} style={{width:'auto', height:'100%', filter:'invert(1)'}}></Image>)}
                        </div>
            ...
            <Container className={editprofilevisible ? '' : 'd-none'} style={{position:'absolute',width:'100%',height:'100vh', zIndex:'1001', paddingTop:'10px',backgroundColor:'#eee', color: 'black', overflow:'hidden'}}>
                ...
                <Col style={{display:'flex', flexDirection:'column', alignItems:'center', width:'100%', height:'100%'}}>
                    ...
                    { newphotoprofile!='' ? (<div onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} style={{width:'80px',height:'80px', display:'flex', alignItems:'center', borderRadius:'50%', border:'2px solid black', overflow:'hidden'}} className='mt-4'>
                ...
                <Container className={opensectionchangephoto ? '' : 'd-none'} style={{position:'absolute', width:'100%', height:'100%', backgroundColor: 'rgba(0, 0, 0, 0.8)', top:'0', left:'0'}}>
                    <Col style={{position:'absolute',width:'100%', bottom: windowSize>=600 ? '0' : '10%', left:'0', backgroundColor:'#232323', padding:'2em'}}>
                        ...
                        <label htmlFor='selectpicture' style={{backgroundColor:'#69696976', width:'70%', marginLeft:'15%', color:'white', border:'0', marginBottom:'1em', textAlign:'left',  paddingTop:'0.5em', paddingBottom:'0.5em', borderRadius:'6px', cursor:'pointer', display:'flex', justifyContent:'center', alignItems:'center'}}><CardImage style={{marginRight:'1em'}}></CardImage>Select new picture</label>
                        ...
                        <XCircle onClick={() => closechangephoto()} style={{position:'absolute', top:'10px', right:'10px',  cursor:'pointer'}} fill='white' size='25'></XCircle>
                    </Col>
                </Container>
            </Container>
            ...
            <Container className={editchannelvisible ? '' : 'd-none'} style={{position:'absolute',width:'100%',height:'100vh', paddingTop:'10px',backgroundColor:'#eee',overflow:'hidden'}}>
                ...
                <Col style={{display:'flex', flexDirection:'column', alignItems:'center', width:'100%', height:'100%'}}>
                    ...
                    <div onMouseMove={handleMouseMovechannel} onMouseUp={handleMouseUpchannel} style={{width:'80px',height:'80px', display:'flex', alignItems:'center', borderRadius:'50%', border:'2px solid black', overflow:'hidden'}}>
                        ...
                        : (<Image src={'/squealer-app'+channel_profile} style={{width:'100%', filter:'invert(1)'}}></Image>)}
                    </div>
                    ...
                    <Container className="text-center text mt-3"  style={{overflowY:'scroll', height:'100vh'}}>
                        ...
                        <Col key={index}>
                            <Card style={{ borderColor:'white', minWidth:'280px', minHeight:'200px', marginBottom:index===(newchannelposts.length-1) ? '150px' : '5%'}}>
                                <Card.Header className='d-flex' style={{justifyContent:'space-between', flexWrap:'wrap'}}>
                                    <CardGroup style={{display:'flex', maxWidth:'280px', overflow:'hidden'}}>
                                    { squeal.photoprofile!='' ? (<div className='me-3' style={{width:'30px',height:'30px', minWidth:'30px', borderRadius:'50%', border:'2px solid black', display:'flex', alignItems:'center', overflow:'hidden'}}>
                ...
                <Container className={opensectionchangephotochannel ? '' : 'd-none'} style={{position:'absolute', width:'100%', height:'100%', top:'0', left:'0', backgroundColor: 'rgba(0, 0, 0, 0.8)', zIndex:'1001'}}>
                    <Col style={{position:'absolute',width:'100%', bottom:windowSize>=600 ? '0' : '10%', left:'0', backgroundColor:'#232323', padding:'2em'}}>
                        ...
                        <label htmlFor='selectpicturechannel' style={{backgroundColor:'#69696976', width:'70%', marginLeft:'15%', border:'0', marginBottom:'1em', textAlign:'left', display:'flex', justifyContent:'center', alignItems:'center',  paddingTop:'0.5em', paddingBottom:'0.5em', borderRadius:'6px', cursor:'pointer', color:'white'}}><CardImage style={{marginTop:'-5px', marginRight:'1em'}}></CardImage>Select new picture</label>
                        ...
                        <XCircle onClick={() => closechangephotochannel()} style={{position:'absolute', top:'10px', right:'10px',  cursor:'pointer'}} fill='white' size='25'></XCircle>
                    </Col>
                </Container>
            </Container>
                ...
                <Col className={confirmaddmessagechannel ? 'text text-center' : 'd-none'} style={{position:'absolute',width:'100%',height:'100%',paddingTop:'3%',backgroundColor:'#eee', overflowY:'scroll', zIndex:'1001'}}>
                    ...
                    <Card className='mt-5' style={{width:'70%', minHeight:'200px', backgroundColor:'white', border:'1px solid #9a6634', borderRadius:'12px', marginLeft:'15%'}}>
                        <Card.Body className='d-flex flex-column' style={{alignItems:'center'}}>
                            ...
                            {capturedVideo && (
                              <div style={{ position: 'relative', width: '300px', maxHeight: '300px', overflow: 'hidden' }}>
                                <video src={capturedVideo} alt="Taken" width="200px" height="100px" controls/>
